...
class Maze(tk.Tk, object):
    ...
    def reset(self):
        ...
        rects_map = np.zeros((4,4))
        r = []
        for i in range(0, self.n_flights):

            test_b = (np.array(self.canvas.coords(rects['rect'+str(i)])[:2],dtype=int) - np.array([5, 5]))//UNIT
            r.extend((np.array(self.canvas.coords(rects['rect'+str(i)])[:2]) - np.array([5, 5]))/UNIT)
            rects_map[test_b[1],test_b[0]] = 1
            test_a = self.canvas.coords(rects['rect' + str(i)])
        # return observation
        return np.array(r)
