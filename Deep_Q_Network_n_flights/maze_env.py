...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        #  'dsu', 'dsd', 'dsl', 'dsr', 'dss',
        #  ...]
        self.against_action_list = [0] * self.n_actions


        for i in range(0,self.n_actions):
            if self.against_action_list[i] != 0:
                continue
            for j in range(i+1, self.n_actions):
                against = True
                for k in range(0, self.n_flights):
                    if int(self.action_space[i][k]) + int(self.action_space[j][k]) != 0:
                        against = False
                        break
                if against:
                    self.against_action_list[i] = j
                    self.against_action_list[j] = i
                    break
                else:
                    continue
    ...
    def step(self, action, allow_restart):
        states = locals()
